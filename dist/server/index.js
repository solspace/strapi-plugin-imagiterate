...
const Replicate__default = /* @__PURE__ */ _interopDefault(Replicate);
...
const collectionName = "imagiterate";
const info = {
  singularName: "imagiterate",
  pluralName: "imagiterates",
  displayName: "Imagiterate Uploads"
};
const pluginOptions = {
  "content-manager": {
    visible: true
...
const imagiterate = imagiterate$1;
var contentTypes = {
  imagiterate
};
...
const controller = ({ strapi: strapi2 }) => ({
  async iterate(ctx) {
    ctx.body = await strapi2.plugin("imagiterate").service("iterate").refineImage(ctx);
  },
  async upload(ctx) {
...
const iterate = ({ strapi: strapi2 }) => ({
  async refineImage(ctx) {
    ...
    const { replicate, model } = getReplicate$1();
    if (replicate.error) return replicate;
    const input = {
      input_image: url,
      prompt
...
async function uploadImage$1(file) {
  const uploadService = getUploadService$1();
  const uploadedFile = await uploadService.upload({
    data: {},
    files: file
  });
...
async function uploadBlob$1(blob) {
  ...
  const uploadedFile = await uploadImage$1(file);
  await fs__default.default.unlink(filePath);
  return uploadedFile;
}
const upload = ({ strapi: strapi2 }) => ({
  async uploadImage(ctx) {
    ...
    let create = await strapi2.documents("plugin::imagiterate.imagiterate").create({
      data: {
