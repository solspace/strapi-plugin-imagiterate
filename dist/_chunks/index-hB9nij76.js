...
const index = {
  register(app) {
    ...
    app.customFields.register({
      ...
      icon: AiIcon,
      components: {
        Input: async () => Promise.resolve().then(() => require("./Input-yLbcJYdi.js")).then((module2) => ({
  ...
  async registerTrads({ locales }) {
    const importedTrads = await Promise.all(
      locales.map((locale) => {
        return __variableDynamicImportRuntimeHelper(/* @__PURE__ */ Object.assign({ "./translations/en.json": () => Promise.resolve().then(() => require("./en-BGsb-QFo.js")) }), `./translations/${locale}.json`, 3).then(({ default: data }) => {
          return {
